...
# Eccentric Strengthening: Tracks slow wrist lowering
class EccentricStrengthening:
    def __init__(self):
        self.counter = 0
        self.stage = "Initial"
        self.angle_threshold_max = 60  # Max wrist extension angle
        self.angle_threshold_min = 20  # Neutral position
        self.last_update = time.time()
        self.lowering_start_time = None  # Track start of lowering for slow movement
    ...
    def track_strengthening(self, hand_landmarks, frame, mp_drawing, mp_hands):
        ...
        # Logic: Count slow lowering from extended to neutral
        if angle > self.angle_threshold_max:
            ...
            self.stage = "Lowering"
            if self.lowering_start_time and current_time - self.lowering_start_time > 2:  # Ensure slow movement (2+ seconds)
...
if __name__ == "__main__":
    print("Starting Eccentric Strengthening Tracker")
    print("Perform slow wrist lowering as if with a light weight. Press 'q' to quit.")

    mp_drawing = mp.solutions.drawing_utils
    mp_hands = mp.solutions.hands
    exercise = EccentricStrengthening()
    ...
    with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5, max_num_hands=1) as hands:
        while cap.isOpened():
            ...
            cv2.imshow('Eccentric Strengthening Tracker', frame)
