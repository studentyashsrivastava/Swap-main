...
# Lateral Stretching: Tracks lateral torso bending on the concave side (right side assumed)
class LateralStretching:
    def __init__(self):
        self.counter = 0
        self.stage = "Initial"
        self.angle_threshold_max = 30  # Max lateral bend angle (comfortable stretch)
        self.angle_threshold_min = 10  # Neutral to slight bend
        self.last_update = time.time()

    def track_stretch(self, landmarks, frame):
        # Use right shoulder, right hip, and a vertical reference (approximated)
        ...
        # Vertical reference point (same x as hip, higher y to simulate vertical axis)
        vertical_ref = [hip[0], hip[1] - 200]  # Approximate vertical point above hip

        # Calculate lateral torso angle (deviation from vertical)
        angle = calculate_angle(shoulder, hip, vertical_ref)

        # Draw lines and points
        cv2.line(frame, shoulder, hip, (0, 255, 0), 2)
        cv2.line(frame, hip, vertical_ref, (255, 0, 0), 2)  # Vertical reference line in blue
        ...
        # Logic: Count a stretch when torso bends laterally to the right and returns
        if angle > self.angle_threshold_max:
            self.stage = "Stretched (Right)"
        elif self.angle_threshold_min < angle < self.angle_threshold_max and self.stage == "Stretched (Right)":
...
if __name__ == "__main__":
    print("Starting Lateral Stretching Tracker")
    print("Perform lateral stretches on the concave side (right side assumed). Press 'q' to quit.")

    mp_drawing = mp.solutions.drawing_utils
    mp_pose = mp.solutions.pose
    exercise = LateralStretching()
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            cv2.imshow('Lateral Stretching Tracker', frame)
