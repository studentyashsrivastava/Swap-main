...
def main():
    ...
    exercise_type = "hammer_curl"  # Egzersiz türünü belirleyin ("hammer_curl", "squat", "push_up", "chair_yoga", "breathing_exercise")
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            if exercise_type == "squat":
                ...
                (counter_right, angle_right, counter_left, angle_left,
                 warning_message_right, warning_message_left, progress_right, progress_left, stage_right, stage_left) = exercise.track_chair_yoga(
                    results.pose_landmarks.landmark, frame)
                layout_indicators(frame, exercise_type,
                                  (counter_right, angle_right, counter_left, angle_left,
                                   warning_message_right, warning_message_left, progress_right, progress_left, stage_right, stage_left))
            elif exercise_type == "breathing_exercise":
                cycle_count, finger_count, phase, warning_message, progress = exercise.track_breathing_exercise(frame)
                layout_indicators(frame, exercise_type, (cycle_count, finger_count, phase, warning_message, progress))
