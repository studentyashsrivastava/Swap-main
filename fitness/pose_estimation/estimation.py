...
class PoseEstimator:
    ...
    def estimate_pose(self, frame, exercise_type):
        ...
        if results.pose_landmarks:
            ...
            if exercise_type == "squat":
                ...
                self.draw_chair_yoga_lines(frame, results.pose_landmarks.landmark)
    ...
    def draw_chair_yoga_lines(self, frame, landmarks):
        # Chair yoga specific lines (arms and spine)
        shoulder_right = [int(landmarks[11].x * frame.shape[1]), int(landmarks[11].y * frame.shape[0])]
        elbow_right = [int(landmarks[13].x * frame.shape[1]), int(landmarks[13].y * frame.shape[0])]
        wrist_right = [int(landmarks[15].x * frame.shape[1]), int(landmarks[15].y * frame.shape[0])]
        hip_right = [int(landmarks[23].x * frame.shape[1]), int(landmarks[23].y * frame.shape[0])]
        ...
        # Draw arm lines
        cv2.line(frame, shoulder_right, elbow_right, (0, 255, 0), 4)
        cv2.line(frame, elbow_right, wrist_right, (0, 255, 0), 4)
        cv2.line(frame, shoulder_left, elbow_left, (0, 255, 0), 4)
        cv2.line(frame, elbow_left, wrist_left, (0, 255, 0), 4)

        # Draw spine lines
        cv2.line(frame, shoulder_right, hip_right, (255, 0, 0), 3)
        cv2.line(frame, shoulder_left, hip_left, (255, 0, 0), 3)
